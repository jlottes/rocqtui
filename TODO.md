...
- [ ] Build current file: `make <target.vo>` for the active file
- [ ] Build file at cursor: build the .vo for the file ^L would jump to
      (parse Require Import, resolve module → .v path → .vo make target)
- [ ] Build dependencies: `make` the .vo targets for all transitive deps
      of the current file. Use `rocqdep` / `coqdep` to discover the dep
      chain, or let `make` handle it (it knows the dependency graph)
- [ ] Build all: `make` in the project root (where _RocqProject lives)
- [ ] Jump to error: parse compiler output for "File X, line N" errors,
      open the file and jump to the error line (reuse ^L / Open_file infra)
- [ ] Cancel build: ^C or Escape to kill the build subprocess
- [ ] Build status in status bar (spinner, "building...", exit code)
- [ ] F5 menu: [f]ile [d]eps [a]ll [c]ursor-file  or similar keybindings
